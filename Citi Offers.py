...
OFFERS_URL = "https://online.citi.com/US/merchantoffers"
...
def _ws(sheet, title: str, headers: Tuple[str, ...]):
    ...
    try:
        ...
        log.warning("Unable to set filter: %s", e)
    return ws
print("_ws loaded")
...
def build_driver():
    ...
    opts.add_argument(f"--user-data-dir={os.getcwd()}/citi_{int(time.time())}")
...
def plus_icons(drv):
    """Return list of 'Add/Activate Offer' buttons (un-enrolled offers)."""
    return drv.find_elements(
        By.XPATH,
        "//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'add offer') or "
        "contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'activate')]"
    )
...
# offer loop
# --------------------------------------------------
def process(drv, wait, holder:str, card_name:str="Citi"):
    """Click every 'Add/Activate Offer' button, scrape details, push to Sheet."""
    ...
    if new_rows:
        OFFER_WS.append_rows(new_rows,value_input_option="RAW",
                             insert_data_option="INSERT_ROWS")
        try:
            OFFER_WS.set_basic_filter()
        except Exception as e:
            log.warning("Unable to refresh filter: %s", e)
...
def main():
    ...
    drv.get(LOGIN_URL)
    print("→ Log in manually, then open Citi Offers page – script will wait.")
    wait_for_login(drv)
    holder = os.getenv("CITI_HOLDER_NAME","Primary")
